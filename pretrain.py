...
from models.miss_pretrain import miss_pretrain
import utils
from utils import warmup_lr_schedule, step_lr_schedule
from data import create_dataset, create_sampler, create_loader

from data.rsna_dataset import rsna_dataset
from data.medicat_dataset import Medicat_dataset
...
def main(args, config):
    ...
    data_loader = DataLoader(dataset=datasets,batch_size=100, num_workers=26, drop_last=False)
    print('dataloader',len(data_loader))
    #### Model #### 
    print("Creating model")
    model = miss_pretrain(image_size=config['image_size'], vit=config['vit'], vit_grad_ckpt=config['vit_grad_ckpt'], 
